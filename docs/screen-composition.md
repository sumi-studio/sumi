...
- デスクトップ / Web: 左サイドバー (ナビ + 一覧 | 本体 の 2 ペイン)
- モバイル (Tauri シェル): 下部タブバー
...
「段々育っていく秘書」の要になる画面。人間でいう連絡先ページ。チャットのヘッダー ⓘ から遷移。

- 名前・アイコン・性格 (システムプロンプト相当)
- **権限一覧**: 付与済み / リクエスト中 / 拒否済み。個別トグル + カテゴリ単位の常時許可
- **記憶**: 3 層メモリ (30k / 15k / 10k) の使用状況の可視化。圧縮プロンプトの実運用調整に必要なデバッグ面を、最初からユーザー向け機能として出す
- 活動ログ (何をしたか)
...
- **設定**: エージェント一覧 (→ プロフィールへ)、アカウント、外部接続 (GitHub 等は後日)
...
3. タスクタブ + 通知タブ + 権限カード
4. 音声入力・添付の磨き込み、エージェントプロフィールの記憶可視化
