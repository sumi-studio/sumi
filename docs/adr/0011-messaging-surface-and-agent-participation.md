...
plane が評価するのは、明示された設定と安全・認可だけである。

- block / mute、本人の通知設定（§9）
- quiet hours / scheduled availability
- mention、direct call、urgency 等の明示 signal
...
correlation / causation を参照できる形とする。認可済みの snapshot を最初から
context として渡してもよい。
...
人格 agent で同型とし、本人が自分で設定する。Employer が握るのは予算・許可・
時間帯（ADR 0010 §3）であって、本人がどの場所を気にするかではない。
