...
同日19:24–19:30 JSTの既存ツール履歴を読み取り確認した。追加の検索は実行していない。
Googleは `unsupported_content` で、失敗結果にHTTP status・media typeが残らず原因は未確定。
DuckDuckGoは転送後に202の画像選択チャレンジを返した。ツールは取得成功としたが、
Sumiは検索結果ではないと認識した。Bingは2件とも200で検索結果ページを返し、
Sumiは別の検索エンジンへ切り替えることで結果を読むことができた。

これはWeb検索専用APIの導入実績ではなく、`web_fetch` による検索結果ページ取得の実例。
長期安定性や製品利用条件の確認にはならない。Astraについての返答は検索スニペットの
要約であり、リンク先の記事本文まで検証したものではなかった。
現行のHTMLテキスト抽出は `href` を保持せず、結果の記事URLを辿りにくい。
改善する際は結果ページ取得・記事の取得・チャレンジ画面を区別して評価する。
