...
Compact の完了順は適用順に使わない。各 layer の永続 `next_batch_seq` と一致する `completed` だけを FIFO で適用し、後続が先に完了した場合は棚で待たせる。L0/L1 の membership 更新、要約の昇格、ジョブの `applied` 化、`next_batch_seq` の前進は同じ SQLite transaction で行う。現仕様は **1 agent = 1 active conversation = 1 `agent.db`** であり、job の `kind` (`compact_l0` / `compact_l1` / `consolidate_l2`) は source layer と1対1に対応するため、`(kind, batch_seq)` はこのDB内では conversation/layer スコープと同値である。この一意制約と状態の比較更新により、再起動・重複通知・ワーカー二重実行でも同じ結果を二重適用しない。将来1 DBで複数conversationを扱う場合は、採番・cursor・一意制約をそろえて `UNIQUE(conversation_id, kind, batch_seq)` へmigrationする。バッチと message の対応は先頭/末尾 ID から推測せず、順序付きの中間表へ全 membership を保存する。
