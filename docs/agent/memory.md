...
- 一人の人格agentは一人の本人として一つのsingle threadを生き、一つのcanonical life logを持つため、記憶の風化も本人の一本の経験上で起きる。run/processをdomain lifetimeにせず、現行runtimeのsingle-active-run schedulingやRustの`Session` actorをidentity contractにしない
...
L0へ入らないErrorのverified contextはprovider再送用ではなく、Error `MessageEnd`から削除intentの`applied`までのcrash-recovery証拠としてだけ保持する。retry/overflowの`RetryScheduled`、terminal Errorの`TurnEnd`、または先行するsupersede/abortと同じtransactionでdeterministicな削除intentをprepareする。disposition以後はcontextを利用対象に戻さず、共通mutation applier/recoveryがrowとitem鍵を破棄して`applied`へ進めるまでruntime control actorをfenceし、その後だけ次attemptの`MessageStart`または`AgentEnd`へ進む。
