...
合計 ~70k トークン以内 (Tool Definitions 次第。80k は超えない) に常に収まる。

## 動作仕様

1. **バッチ分割**: L0 は下限 5k トークンでバッチに分割する。メッセージやツールコール/結果の途中では切らず、きりのいい境界で切る
2. **先回り Compact**: バッチが確定したら即座に非同期で Compact (LLM 要約) し、結果は棚に保存する。この時点では適用せず、L0 からも消さない
...
6. **適用タイミング**: 溢れ処理の適用は「次の API コールの直前」であればいつでもよい。ツールコールの切れ目でも構わない。ユーザーのメッセージ送信直後 (TTFT が見える瞬間) には走らせない
