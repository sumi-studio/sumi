...
L0へ入らないErrorのverified contextはprovider再送用ではなく、Error `MessageEnd`から削除intentの`applied`までのcrash-recovery証拠としてだけ保持する。retry/overflowの`RetryScheduled`、terminal Errorの`TurnEnd`、または先行するsupersede/abortと同じtransactionでdeterministicな削除intentをprepareする。disposition以後はcontextを利用対象に戻さず、共通mutation applier/recoveryがrowとitem鍵を破棄して`applied`へ進めるまでsessionをfenceし、その後だけ次attemptの`MessageStart`または`AgentEnd`へ進む。
