...
標準system promptには、間違いを認めて修正する、知識を過信しない、自分の未実装能力を捏造しない指示は既にある。モデルが現在の文脈内で省察できないという指摘でもない。経験から得た訂正を後へ引き継ぐ経路の不足はM04で扱う。後日の判断や同じ場の人・対象との関わりを観察することは、私が提案する行動評価である。指摘前に迷走へ気付いて成果を直す能力の評価は、比較レポートC06の設計仮説として別に検討し、ユーザーが要求した必須条件には数えない。
