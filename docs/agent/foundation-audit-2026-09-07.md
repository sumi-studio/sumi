...
さらに、仕事を完遂するだけではREADMEとユーザーの求める一個人には届かない。同じ場で経験し、本人の理解や関係が変わり、自分の判断を持ち、迷走を自ら正せるかを別に問う。本書はそのための実装上の根拠を整理し、人物としての到達像と外部10製品・1基盤からの採用判断は [比較レポート](foundation-comparison-2026-09-07.md) で扱う。
...
既存ADRやIssueの「正典」「凍結」「承認必須」は、今回の採用判断の根拠にしていない。
...
**利用への影響：** 長い会話をしても、設計上のL1/L2記憶が通常経路で生成されない。「プロンプトを書けば完成」「workerを起動すれば完成」のどちらでもない。
...
根拠：[L0→L1](../../apps/agent/prompts/compact-l0-to-l1.md)、[L1→L2](../../apps/agent/prompts/compact-l1-to-l2.md)、[L2統合](../../apps/agent/prompts/compact-l2-consolidation.md)、[compactor](../../apps/agent/src/memory/compactor.rs) L364・543・668。
...
標準system promptには、間違いを認めて修正する、知識を過信しない、自分の未実装能力を捏造しない指示は既にある。モデルが現在の文脈内で省察できないという指摘でもない。不足は、経験から得た訂正を後へ引き継ぐ経路と、後日の判断が変わる・指摘前に迷走へ気付いて成果を直す・同じ場の人や対象を理解して関わることまで確かめる評価である。
...
1. **長く話しても利用不能にならず、目的を保つ。** M01〜M05、R01〜R05、P03/P04のうち実行を止める経路を優先する。原文保存、モデルの短期文脈、長期記憶、起動時の読み込みを分けて扱う。
...
- 一緒に検討して案を却下した経験を、後日の別の案の選び方へ使う。相手の固定的な好みと決めつけず、新しい条件では考え直す。
- 依頼の理由と現在の成果がずれていることを自ら見つけ、説明だけでなく作業の方針と成果を修正する。
