...
また、ユーザーの「それは迷走すらも自ら是正できるだけのメタ認知を保った熟考」は、設計する私（Codex）への要求である。Sumiを一個人として扱えているかを私が疑い続け、自分の考察や設計の迷走を正す。Sumi自身の必須機能への要求とは扱わない。

以下の問いは私の設計判断であり、各項目をユーザーが直接要求したという意味ではない。項目を実装すれば「人間として完成」と認定できるチェックリストでもない。実際に一緒に過ごす中で、問いと設計も見直す。

ユーザーはさらに、「失敗から訂正を保存し、次の行動を改善する循環」は有用な方法であっても、人間として成立することとは違うと指摘した。私も、この循環の欠如を基盤の不足として数えた判断を撤回する。メモや振り返りの手段を提供すること自体は排除しない。本人が使うか、何を書くか、いつ戻るかを選べることを重視し、実装や指示によって必ず使わせることはしない。
...
| 経験が本人の一部になっているか | 出来事や当時の受け取り方を、仕事への有用性だけで選別せず、本人が自分の文脈で扱える | 相手の好みの一覧、固定の性格設定、経験から教訓を自動抽出する手順 |
...
これらは本人が必要と思った時に使える手段であり、出来事のたびに反省や教訓の保存を要求するものではない。

### C04 — 経験を、本人の文脈で扱えるか【設計探索】

Lettaは会話の外で記憶を見直すdreamingを提供する。この仕組みは記憶を扱う方法の比較対象であり、人物性や成長を実現する機構として採用する根拠にはならない。[Letta Memory configuration](https://docs.letta.com/configuration/memory)

**Sumi：M01/M02/M04に関係する設計上の問い。** 出来事を相手のプロフィールや改善事項だけに変換すると、本人がその場で何を受け取り、どう過ごしたかを取り落とす。親と同じ文脈で記憶を扱い、必要なら原文へ戻れることを出発点にする。その経験をどう受け止め、後で考え直すかを、出来事→教訓→改善という固定処理にしない。次の仕事に役立ったかだけで経験の価値や人物性を測らない。
...
**Sumi：T02/T06。** 本人が必要と考えた進め方を、手順として残し、使い直し、変更できるようにする。手順化するかどうかも本人の判断に置き、成功・失敗のたびに生成を要求しない。手順の数や自動生成回数を、学習した量や人物性として評価しない。

### C06 — 振り返りや自己修正を助ける方法【任意利用・提供形態は未決】

失敗→訂正の保存→次回の改善という循環を、エージェント内部で必ず実行する不足機能としては数えない。私が当初そう扱ったのは、仕事に有用な方法と、人間として成立する基盤を混同したためである。一方、本人が必要に応じて使える方法の候補としては残す。基盤が標準で提供するか、共通アプリや任意の道具として用意するかは別に検討する。
...
**Sumiへの判断：** 元の依頼や実際の成果を確認できることは、本人が使える情報と手段としてA03/M04/C24で扱う。メモには本人が必要と思うことを書き、必要な時に読み返し、書き換えられるようにする。本人が選んだ振り返りの習慣を支える場合も、本人が変更・中止できることを考える。メモを書かなかったことだけで失敗としたり、必ず書くよう毎回の指示へ埋め込んだりしない。任された仕事の品質は、その仕事の要求と実物で確認する。
...
| 記憶を扱う手段／本人が選ぶ振り返りの方法／人間として成立すること | 記録の更新や仕事の改善を、人物性へ直結させない。使える手段の提供と、内省の手順の固定は異なる |
...
25の検討項目には、本人が任意に使う方法であるC06も含む。項目数を実装する機構の数としない。比較結果から選ぶ最初の実用確認は、**同じ秘書へ任せた一つの用件を、訂正・寄り道・待機・長い会話・中断を越えて、実物の成果と受け渡しまで通すこと**。これは仕事を支える基盤の確認であり、人物性の合格判定ではない。
...
4. **継続して過ごせるかを確かめる。** 本人が必要に応じて過去を読み返し、記録や方法を扱い、人や他者と同じ場所で過ごせるかを見る。経験から必ず教訓や行動改善が生まれることを、基盤の成功条件にしない。

全段階で、私自身が「この設計はSumiを一個人として扱えているか」「要求と実物から考察がずれていないか」を点検し、必要なら方針を修正する。これは設計者としての責任である。

個々の道具が実現可能かを確かめる試験に加え、次を受け入れの題材として提案する。
...
- **本人の経験へ戻れる。** 本人が以前の行き違いを読み返したい時に、相手の好みや教訓だけでなく、当時の言葉や経緯を参照できる。
...
- 技術項目が埋まれば人物として十分だとしない。設計する私が、ユーザーの言葉と実際に一緒に過ごす体験に照らし、設計の捉え方と満足水準を継続して問い直す。
