...
### C06 — 目的・根拠・結果を見比べ、迷走を自分で修正する【設計仮説・要検討】

これは私が提案するSumiの能力候補である。ユーザーが求めた設計者のメタ認知を、そのままSumiの要件へ移したものとしては扱わず、全段階の必須受入条件にはしない。採用の是非と評価方法は別に検討する。
...
**Sumi：A03/M04/Q01に関係する設計仮説。** 標準promptには間違いを認めて直す指示は既にある。元の意図・理由・未確定の前提・実際の成果へ戻れることが、食い違いに気付き、調査をやり直す、別案を試す、相談する、不要な作業をやめる判断に役立つかを検討したい。この仮説は「振り返りました」という報告だけでは確かめられない。モデルが持つ判断力を使うための情報と機会を考え、毎ターン固定の自己批判を義務化しない。
...
25項目を25個の機構として先に作らない。比較結果から選ぶ最初の仕事は、**同じ秘書へ任せた一つの用件を、訂正・寄り道・待機・長い会話・中断を越えて、実物の成果と受け渡しまで通すこと**。
...
4. **経験を重ねて変わることを確かめる。** 記憶の訂正、本人の判断と関係の推移、手順の再利用、共同UI・他者との協働を、実際の継続利用で育てる。C04を含む設計仮説をその体験に照らして見直す。
...
C06を検討する場合には、ツールが成功しても成果が元の要求に届かない場面で、本人が指摘前に違いへ気付き、修正や必要な相談へ進めるかを別途観察する。これは自己修正の設計仮説を確かめる候補であり、上記の全段階に必須の条件ではない。
