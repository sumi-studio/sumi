...
調査日：2026-09-07。実装の根拠と30件の指摘は [実装監査](foundation-audit-2026-09-07.md) に分けた。本書のM/A/T/R/P/U/Q番号はその指摘IDを参照する。
...
**Sumi：M01〜M03。** 部品はあるが本番の生成・適用が未接続で、現在は古い入力を外す。最小の改善は一つの圧縮経路を実稼働させ、依頼の理由、訂正、本人の判断、未完了と結果不明を保って次の行動へ使うこと。既存の三層構成を完成させること自体は目的にしない。短期の引き継ぎと、本人が長く覚えておく経験は別に考える。
...
**Sumi：T05/A01〜A03。** 相手が別の秘書なら、その人の名前・立場・引き受けた範囲・返事を持つ共同作業として扱う。補助計算なら道具として寿命と結果を扱う。一人の本人を複数の独立した続きへ分裂させて後で統合することを、標準の高速化にはしない。
...
| 一見同じに見えるもの | 分ける理由 |
|---|---|
| Compact／原履歴検索／長期記憶／手順／未完了の約束 | 今考えられる量、思い出す手段、現在の理解、使える方法、引き受けた責任は異なる |
...
1. **今ある土台で続けられる状態へ直す。** C01/C02/C13と、起動・provider・保守失敗が本人全体を止める経路。既存履歴を使い、全原文を起動時に読まない。
...
- AI秘書を、独立した使い捨てsessionの束として扱う構造をそのまま持ち込まない。
- 人間として扱うために、わざと忘れる・間違える・気分を演じる機構は作らない。
