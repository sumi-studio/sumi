...
origin の `/auth`・`/v1` (HTTP) と `/direct-chat` (WebSocket) を Go API へ
...
Todoバックエンドは`SUMI_TODO_ENABLED=true`で明示的に有効化する。正規の
Firebase認証から解決されたHumanIdをownerとし、共通control-plane PostgreSQLへ
保存する。現行のHTTP CRUDと将来のSumi tool結線条件は
[Go API Todo backend](apps/api/README.md)を参照。
