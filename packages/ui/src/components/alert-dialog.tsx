...
export function AlertDialogContent({
  ...
  return (
    <ScopedPortal component={AlertDialogPrimitive.Portal}>
      {/* A modal confirmation must sit above the surfaces that raised it,
          including z-60 popovers like the settings panel. */}
      <AlertDialogPrimitive.Backdrop className="fixed inset-0 z-70 bg-black/20 transition-opacity duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0" />
      <AlertDialogPrimitive.Popup
        data-slot="alert-dialog-content"
        className={cn(
          "fixed top-1/2 left-1/2 z-70 grid w-[calc(100%-2rem)] max-w-sm -translate-x-1/2 -translate-y-1/2 gap-4 rounded-2xl bg-popover p-5 text-popover-foreground shadow-xl ring-1 ring-foreground/10 outline-none transition duration-150 data-ending-style:scale-95 data-ending-style:opacity-0 data-starting-style:scale-95 data-starting-style:opacity-0",
          className,
        )}
        {...props}
      />
