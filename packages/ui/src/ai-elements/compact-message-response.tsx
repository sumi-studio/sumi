...
/** Minimal mdast shape used by the math safety transforms. */
interface MdNode {
  type: string;
  value?: string;
  children?: MdNode[];
...
function splitDisplayMathLines(
  ...
  const flush = () => {
    ...
    while (line.length > 0 && line.at(-1)?.type === "break") line.pop();
...
/** Keep currency prose literal and treat a whole $$…$$ line as display math. */
...
async function copyText(text: string): Promise<boolean> {
  try {
    ...
    // Permission denial and insecure origins can still use the selection path.
  }
