...
for name in SUMI_DB_URL SUMI_APP_SHA SUMI_API_IMAGE SUMI_POSTGRES_IMAGE; do require_value "${name}"; done
[[ "${SUMI_APP_SHA}" =~ ^([0-9a-f]{40}|[0-9a-f]{64})$ ]] || { printf 'SUMI_APP_SHA is not exact\n' >&2; exit 2; }
for name in SUMI_API_IMAGE SUMI_POSTGRES_IMAGE; do
  [[ "${!name}" =~ ^[a-z0-9./:_-]+@sha256:[0-9a-f]{64}$ ]] || { printf '%s is not an exact image digest\n' "${name}" >&2; exit 2; }
done
for name in SUMI_BACKUP_WORK_ROOT SUMI_ATTACHMENT_ROOT; do require_directory "${name}"; done
...
for name in SUMI_MIGRATE_BIN SUMI_PSQL_BIN SUMI_PG_DUMP_BIN SUMI_TAR_BIN SUMI_QUIESCE_HELPER SUMI_RESUME_HELPER SUMI_ENCRYPT_HELPER SUMI_HANDOFF_HELPER; do
...
"${SUMI_PSQL_BIN}" "${SUMI_DB_URL}" -X -v ON_ERROR_STOP=1 -At -F $'\t' \
...
"${SUMI_PG_DUMP_BIN}" --format=custom --file="${snapshot_dir}/database.dump" "${SUMI_DB_URL}"
"${SUMI_TAR_BIN}" --create --file="${snapshot_dir}/attachments.tar" --directory="${SUMI_ATTACHMENT_ROOT}" .
node "${script_dir}/snapshot-manifest.mjs" create \
  "${snapshot_dir}" "${snapshot_id}" "${SUMI_APP_SHA}" \
  "${SUMI_API_IMAGE}" "${SUMI_POSTGRES_IMAGE}" >/dev/null
...
"${SUMI_TAR_BIN}" --create --file="${bundle}" --directory="${snapshot_dir}" \
  database.dump attachments.tar attachment-rows.tsv attachments.manifest.json migration-manifest.json snapshot.json
