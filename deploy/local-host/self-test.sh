...
[[ -x $T_PREFIX/bin/sumi-local-move-bin ]] || fail "move/return binary missing"
[[ $(readlink "$HOME/.local/bin/sumi-local-move") == "$T_PREFIX/bin/sumi-local-move" ]] || fail "move/return shim missing"
"$HOME/.local/bin/sumi-local-move" help >/dev/null || fail "installed move/return command cannot run"
...
[[ ! -L $HOME/.local/bin/sumi-local-move ]] || fail "uninstall left the owned move shim"
...
"$HOME/.local/bin/sumi-local-move" return-status >/dev/null || fail "pack-installed return-status failed"
