...
mod tests {
    ...
    #[test]
    fn broker_delegates_project_evaluate_and_resolve() {
        let read = CanonicalAction::from_tool_call(
            PathBuf::from("/workspace"),
            "read_file",
            &args(json!({"path": "notes.txt"})),
        ...
        // evaluate() must delegate to the underlying Policy.
        let broker = default_broker();
        assert!(broker.evaluate(&read).is_allow());

        // project() must delegate to the underlying projector and reflect the
        // same literal, non-secret path.
        let ReviewProjection::Reviewable(review) = broker.project(&read) else {
            panic!("expected reviewable projection for a plain workspace read");
        };
        assert_eq!(review.tool, "read_file");

        // resolve() must delegate to Policy::resolve; a Deny decision is
        // independent of the policy's own evaluation of the action.
        let resolved = broker.resolve(&read, UserDecision::Deny);
        assert!(matches!(resolved, ResolvedDecision::Deny));

        // An ApproveOnce decision for an allowed action resolves to
        // ApproveOnce rather than being rejected.
        let resolved_once = broker.resolve(&read, UserDecision::ApproveOnce);
        assert!(matches!(resolved_once, ResolvedDecision::ApproveOnce));
    }
}
