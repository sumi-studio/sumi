...
mod tests {
    ...
    #[test]
    fn approval_rule_matches_requires_matching_tool_prefix_and_permissions() {
        let action = CanonicalAction::from_tool_call(
            PathBuf::from("/workspace"),
            "read_file",
            &args(json!({"path": "notes.txt"})),
        ...
        let base = ApprovalRule {
            id: "r".to_owned(),
            tool: "read_file".to_owned(),
            literal_prefix: vec!["read_file".to_owned(), "notes.txt".to_owned()],
            effect: RuleEffect::Allow,
            workspace_only: true,
            allowed_permissions: vec![Permission::ReadWorkspace],
            allowed_network_domains: vec![],
        ...
        assert!(
            base.matches(&action, &action.argv, &workspace),
            "a rule matching tool, prefix, and permissions should match"
        );

        let wrong_tool = ApprovalRule {
            tool: "write_file".to_owned(),
            ..base.clone()
        };
        assert!(!wrong_tool.matches(&action, &action.argv, &workspace));

        let wrong_prefix = ApprovalRule {
            literal_prefix: vec!["read_file".to_owned(), "other.txt".to_owned()],
            ..base.clone()
        };
        assert!(!wrong_prefix.matches(&action, &action.argv, &workspace));

        let insufficient_permissions = ApprovalRule {
            allowed_permissions: vec![],
            ..base
        };
        assert!(!insufficient_permissions.matches(&action, &action.argv, &workspace));
    }

    #[test]
    fn approval_rule_matches_workspace_only_flag_controls_escape_enforcement() {
        let action = CanonicalAction::from_tool_call(
            PathBuf::from("/workspace"),
            "read_file",
            &args(json!({"path": "/etc/passwd"})),
        )
        .expect("a canonical action does not itself enforce workspace membership");
        let workspace = PathBuf::from("/workspace");

        let scoped = ApprovalRule {
            id: "scoped".to_owned(),
            tool: "read_file".to_owned(),
            literal_prefix: vec!["read_file".to_owned(), "/etc/passwd".to_owned()],
            effect: RuleEffect::Allow,
            workspace_only: true,
            allowed_permissions: vec![Permission::ReadWorkspace],
            allowed_network_domains: vec![],
        ...
        assert!(
            !scoped.matches(&action, &action.argv, &workspace),
            "workspace_only rule must reject an action whose affected path escapes the workspace"
        );

        let unscoped = ApprovalRule {
            workspace_only: false,
            ..scoped
        };
        assert!(
            unscoped.matches(&action, &action.argv, &workspace),
            "matches() only checks cwd (not affected_paths) when workspace_only is false"
        );
    }

    #[test]
    fn from_effects_selects_strictest_effect_and_dedups_matched_rules() {
        let decision = from_effects(vec![
            (RuleEffect::Allow, "a".to_owned()),
            (RuleEffect::Forbidden, "b".to_owned()),
            (RuleEffect::NeedsApproval, "a".to_owned()),
        ]);
        match decision {
            PolicyDecision::Forbidden {
                matched_rules,
                ...
                assert_eq!(matched_rules, vec!["a".to_owned(), "b".to_owned()]);
                assert!(reason.contains('a') && reason.contains('b'));
            }
            other => panic!("expected Forbidden, got {other:?}"),
        }

        let allow_only = from_effects(vec![(RuleEffect::Allow, "only".to_owned())]);
        assert!(matches!(
            allow_only,
            PolicyDecision::Allow { matched_rules } if matched_rules == vec!["only".to_owned()]
        ));
    }

    #[test]
    fn combine_forbidden_dominates_and_preserves_first_matching_reason() {
        let forbidden_a = PolicyDecision::Forbidden {
            matched_rules: vec!["a".to_owned()],
            reason: "reason-a".to_owned(),
        };
        let forbidden_b = PolicyDecision::Forbidden {
            matched_rules: vec!["b".to_owned()],
            reason: "reason-b".to_owned(),
        };
        match combine(forbidden_a, forbidden_b) {
            PolicyDecision::Forbidden {
                matched_rules,
                reason,
            } => {
                assert_eq!(matched_rules, vec!["a".to_owned(), "b".to_owned()]);
                assert_eq!(reason, "reason-a", "the first Forbidden reason should win");
            }
            other => panic!("expected Forbidden, got {other:?}"),
        }

        let needs = PolicyDecision::NeedsApproval {
            matched_rules: vec!["x".to_owned()],
            reason: "needs-x".to_owned(),
        };
        let forbidden = PolicyDecision::Forbidden {
            matched_rules: vec!["y".to_owned()],
            reason: "forbidden-y".to_owned(),
        };
        match combine(needs, forbidden) {
            PolicyDecision::Forbidden {
                matched_rules,
                reason,
            } => {
                assert_eq!(matched_rules, vec!["x".to_owned(), "y".to_owned()]);
                assert_eq!(
                    reason, "forbidden-y",
                    "Forbidden must dominate NeedsApproval regardless of argument order"
                );
            }
            other => panic!("expected Forbidden, got {other:?}"),
        }
    ...
    #[test]
    fn is_broad_prefix_rejects_prefixes_shorter_than_two_tokens() {
        assert!(is_broad_prefix(&[]), "an empty prefix must be broad");
        assert!(
            is_broad_prefix(&["ls".to_owned()]),
            "a single-token prefix must be broad regardless of the command"
        );
        assert!(
            !is_broad_prefix(&["git".to_owned(), "status".to_owned()]),
            "a narrow, modeled two-token prefix should not be broad"
        );
    }
}
