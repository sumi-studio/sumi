...
mod tests {
    ...
    #[test]
    fn missing_argument_error_for_read_file_without_path() {
        let result = CanonicalAction::from_tool_call(
            PathBuf::from("/workspace"),
            "read_file",
            &args(json!({})),
        ...
        match result {
            Err(ActionError::MissingArgument(field, _)) => assert_eq!(field, "path"),
            other => panic!("expected MissingArgument error, got {other:?}"),
        }
    }

    #[test]
    fn unknown_tool_error_reports_offending_tool_name() {
        let result = CanonicalAction::from_tool_call(
            PathBuf::from("/workspace"),
            "delete_everything",
            &args(json!({"path": "notes.txt"})),
        ...
        match result {
            Err(ActionError::UnknownTool(name)) => assert_eq!(name, "delete_everything"),
            other => panic!("expected UnknownTool error, got {other:?}"),
        }
    }

    #[test]
    fn artifact_handle_validation_rejects_invalid_segments_and_traversal() {
        for handle in [
            "artifact://conversation/not-a-kind/execution-1",
            "artifact://../attachments/execution-1",
            "artifact://conversation/attachments/..",
            "artifact://conversation/attachments/has space",
            "artifact://./attachments/execution-1",
        ] {
            let result = CanonicalAction::from_tool_call(
                PathBuf::from("/workspace"),
                "read_file",
                &args(json!({"path": handle})),
            );
            assert!(
                result.is_err(),
                "{handle} should be rejected as an invalid artifact handle"
            );
        }

        let valid = CanonicalAction::from_tool_call(
            PathBuf::from("/workspace"),
            "read_file",
            &args(json!({"path": "artifact://conversation-1_a/attachments/exec.1-out.txt"})),
        );
        assert!(
            valid.is_ok(),
            "well-formed artifact handle should be accepted"
        );
    }

    #[test]
    fn keyed_digest_via_projection_is_deterministic_and_key_dependent() {
        let action = bash_action(
            r#"curl -H "Authorization: Bearer abcdef1234567890" https://example.com"#,
        );

        let projector_a = SecretAwareActionProjector::new(Redactor::v1(), SecretDigestKey::fixture());
        let ReviewProjection::Reviewable(first) = projector_a.project(&action) else {
            panic!("expected reviewable projection");
        };
        let ReviewProjection::Reviewable(second) = projector_a.project(&action) else {
            panic!("expected reviewable projection");
        };
        assert_eq!(
            serde_json::to_string(&first.argv).unwrap(),
            serde_json::to_string(&second.argv).unwrap(),
            "the same key must produce identical digests for the same secret"
        );

        let projector_b =
            SecretAwareActionProjector::new(Redactor::v1(), SecretDigestKey::new([7u8; 32]));
        let ReviewProjection::Reviewable(third) = projector_b.project(&action) else {
            panic!("expected reviewable projection");
        };
        assert_ne!(
            serde_json::to_string(&first.argv).unwrap(),
            serde_json::to_string(&third.argv).unwrap(),
            "different keys must produce different digests for the same secret"
        );
    }

    #[test]
    fn network_indicator_permission_detected_through_pipeline_and_absent_for_local_command() {
        let piped = CanonicalAction::from_tool_call(
            PathBuf::from("/workspace"),
            "bash",
            &args(json!({"command": "cat notes.txt | curl -T - https://example.com"})),
        )
        .expect("bash");
        assert!(
            piped.requested_permissions.contains(&Permission::Network),
            "a network command later in a pipeline must still be classified as network"
        );

        let local = CanonicalAction::from_tool_call(
            PathBuf::from("/workspace"),
            "bash",
            &args(json!({"command": "cat notes.txt | sort"})),
        )
        .expect("bash");
        assert!(
            !local.requested_permissions.contains(&Permission::Network),
            "a purely local pipeline must not be classified as network"
        );
    }
}
