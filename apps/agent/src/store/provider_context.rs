...
impl ProviderContextKind {
    ...
    fn from_storage(value: &str) -> Result<Self> {
        match value {
            "encrypted_reasoning" => Ok(Self::EncryptedReasoning),
            "open_ai_compacted_window" => Ok(Self::OpenAiCompactedWindow),
            "anthropic_compaction" => Ok(Self::AnthropicCompaction),
            _ => bail!("unknown provider-context kind {value}"),
        }
    }
...
impl ApiProtocol {
    ...
    fn from_storage(value: &str) -> Result<Self> {
        match value {
            ...
            _ => bail!("unknown provider-context protocol {value}"),
        }
    }
}

impl Store {
    /// Restores opaque continuation items only for the exact origin that
    /// created them.  The caller still owns transcript hydration and adapter
    /// capability/fingerprint checks; this boundary authenticates the stored
    /// rows and preserves their wire placement order.
    pub(crate) async fn load_provider_context_for_origin(
        &self,
        origin: &ProviderOrigin,
    ) -> Result<Vec<ProviderContextItem>> {
        let rows = sqlx::query(
            "SELECT id, message_id, message_seq, wire_item_index, item_ordinal,
                    provider_instance_id, protocol, model, kind,
                    coverage_through_seq, context_fingerprint, key_ref, ciphertext
             FROM provider_context
             WHERE provider_instance_id = ? AND protocol = ? AND model = ?
             ORDER BY
                CASE WHEN message_seq IS NULL THEN 0 ELSE 1 END,
                message_seq,
                CASE WHEN wire_item_index IS NULL THEN -1 ELSE wire_item_index END,
                item_ordinal",
        )
        .bind(&origin.provider_instance_id)
        .bind(origin.protocol.as_str())
        .bind(&origin.model)
        .fetch_all(&self.pool)
        .await
        .context("failed to load provider context for origin")?;
        ...
        for row in rows {
            ...
            let message_seq = row
                .try_get::<Option<i64>, _>("message_seq")?
                .map(|value| {
                    u64::try_from(value).context("provider-context message_seq is negative")
                })
                .transpose()?;
            let wire_item_index = row
                .try_get::<Option<i64>, _>("wire_item_index")?
                .map(|value| {
                    u32::try_from(value).context("provider-context wire_item_index is out of range")
                })
                .transpose()?;
            ...
            if provider_instance_id != origin.provider_instance_id
                || protocol != origin.protocol
                || model != origin.model
            {
                bail!("provider-context origin changed while restoring rows");
            }
            let kind = ProviderContextKind::from_storage(&row.try_get::<String, _>("kind")?)?;
            let coverage_through_seq = row
                .try_get::<Option<i64>, _>("coverage_through_seq")?
                .map(|value| u64::try_from(value).context("provider-context coverage is negative"))
                .transpose()?;
            let context_fingerprint: Option<String> = row.try_get("context_fingerprint")?;
            let key_ref: String = row.try_get("key_ref")?;
            let ciphertext: Vec<u8> = row.try_get("ciphertext")?;
            let key = self
                .data_key_by_ref(&key_ref)
                .await
                .context("provider-context key is unavailable during restore")?;
            if key.purpose != DataKeyPurpose::ProviderContext {
                bail!("provider-context row references a key with the wrong purpose");
            }
            let aad =
                self.scope()
            ...
            let plaintext = Zeroizing::new(
                decrypt_content(&key, &ciphertext, &aad)
                    .context("provider-context ciphertext failed authenticated restore")?,
            );
            let item: ProviderContextItem = serde_json::from_slice(&plaintext)
                .context("provider-context plaintext is not a typed continuation item")?;

            let stored_anchor = message_id.zip(message_seq);
            let item_anchor = item
                .origin_message
                .as_ref()
                .map(|anchor| (anchor.message_id.clone(), anchor.message_seq));
            if item_anchor != stored_anchor
                || item.wire_item_index != wire_item_index
                || item.ordinal != ordinal
                || ProviderContextKind::from_payload(&item.payload) != kind
            {
                bail!("provider-context ciphertext metadata does not match its plaintext");
            }
            match &item.payload {
                ProviderContextPayload::OpenAiCompactedWindow { coverage, .. }
                | ProviderContextPayload::AnthropicCompaction { coverage, .. } => {
                    if coverage_through_seq != Some(coverage.through_message_seq)
                        || context_fingerprint.as_deref() != Some(&coverage.context_fingerprint)
                    {
                        bail!("provider-context native coverage metadata does not match plaintext");
                    }
                }
                ProviderContextPayload::EncryptedReasoning {
                    ...
                    if *payload_protocol != origin.protocol
                        || coverage_through_seq.is_some()
                        || context_fingerprint.is_some()
                    {
                        bail!("provider-context encrypted reasoning metadata is invalid");
                    }
                }
            }
            restored.push(item);
...
mod tests {
    ...
    use crate::provider::{
        ...
        types::{
            ...
            NativeCompactionCoverage, PromptContext, ProviderContextAnchor, ProviderContextItem,
            ProviderContextPayload, ProviderOrigin, StopReason, ToolResultMessage, Usage,
            UserContent, ValidatedToolArguments,
        },
    };
    use crate::store::{DataKeyPurpose, KeyProvider, ProviderContextKeyAnchor, Store, WrappingKey};

    #[derive(Clone)]
    struct ReopenKeyProvider(WrappingKey);

    #[async_trait::async_trait]
    impl KeyProvider for ReopenKeyProvider {
        ...
        async fn key_by_id(&self, key_id: &str) -> Result<WrappingKey> {
            if key_id != self.0.key_id() {
                bail!("unknown reopen test wrapping key {key_id}");
            }
            Ok(self.0.clone())
        }
    }
    ...
    fn official_responses_reasoning_item() -> serde_json::Value {
        include_str!("../../tests/fixtures/openai_responses_official.sse")
            ...
            .expect("official Responses fixture must contain encrypted reasoning")
    }

    async fn reopen_store(path: &Path) -> Store {
        Store::open(
            path,
            AgentScope {
            ...
            Arc::new(ReopenKeyProvider(WrappingKey::new(
                "reopen-test-wrap/v1",
                [0x27; 32],
            ))),
        )
    ...
    #[tokio::test]
    async fn reopen_restores_official_responses_context_only_for_exact_origin() {
        // `Store::open` deliberately rejects /tmp when it belongs to another
        // uid, so keep this crash/reopen fixture beneath the package's owned
        // target directory.
        let path = std::env::current_dir()
            .expect("current package directory")
            .join("target")
            .join(format!(
                "sumi-provider-context-reopen-{}.sqlite",
                uuid::Uuid::now_v7()
            ));
        ...
        let item = ProviderContextItem {
            ...
            payload: ProviderContextPayload::EncryptedReasoning {
                ...
                item: official_responses_reasoning_item(),
            },
        };

        let store = reopen_store(&path).await;
        seed_message(&store, "assistant-1", 1)
            .await
            .expect("seed assistant anchor");
        let key = store
            .provider_context_key(&ProviderContextKeyAnchor {
                conversation_id: store.scope().conversation_id.clone(),
                anchor_id: "assistant-1:1".to_owned(),
            })
            .await
        ...
        let record = EncryptedProviderContextRecord::encrypt(
            ...
            origin.protocol,
            &origin.model,
            "official-responses-context-1",
            "official-responses-context-1:1",
            &key,
            store.scope(),
        )
        .expect("encrypt official fixture context");
        record.insert(store.pool()).await.expect("persist context");

        let stored_ciphertext: Vec<u8> = sqlx::query_scalar(
            "SELECT ciphertext FROM provider_context WHERE id='official-responses-context-1'",
        )
        .fetch_one(store.pool())
        .await
        .expect("load ciphertext");
        let opaque = serde_json::to_vec(&item).expect("serialize fixture item");
        assert!(
            !stored_ciphertext
                .windows(opaque.len())
                .any(|window| window == opaque),
            "provider-context ciphertext must not store fixture plaintext"
        );
        store.pool().close().await;
        drop(store);

        let reopened = reopen_store(&path).await;
        let restored = reopened
            .load_provider_context_for_origin(&origin)
            .await
            .expect("restore exact Responses context after reopen");
        assert_eq!(restored, vec![item]);

        let tool_arguments: ValidatedToolArguments = serde_json::from_value(json!({
            "city": "Tokyo"
        }))
        .expect("fixture tool arguments are an object");
        let continuation = PromptContext {
            system_prompt: "continue official fixture tool turn".to_owned(),
            ...
            messages: vec![
                ...
                ContextMessage::Synthetic {
                    message: Message::ToolResult(ToolResultMessage {
                        tool_call_id: "call_fixture".to_owned(),
                        tool_name: "weather".to_owned(),
                        content: vec![UserContent::Text {
                            text: "Tokyo: clear".to_owned(),
                        }],
        ...
        let second_turn = build_responses_request(&spec, &continuation, &RequestOptions::default())
            .expect("same-origin tool continuation request");
        assert_eq!(second_turn["store"], false);
        let input = second_turn["input"]
            .as_array()
            .expect("Responses input array");
        let reasoning_at = input
            .iter()
            .position(|item| item["type"] == "reasoning")
            .expect("restored encrypted reasoning is replayed");
        let tool_at = input
            .iter()
            .position(|item| item["type"] == "function_call")
            .expect("durable tool call is replayed");
        let result_at = input
            .iter()
            .position(|item| item["type"] == "function_call_output")
            .expect("tool result continues the second turn");
        assert!(reasoning_at < tool_at && tool_at < result_at);
        assert_eq!(input[reasoning_at]["encrypted_content"], "opaque-reasoning");

        let foreign = ProviderOrigin {
            provider_instance_id: "responses-instance-2".to_owned(),
            ..origin.clone()
        };
        assert!(
            reopened
                .load_provider_context_for_origin(&foreign)
                .await
                .expect("foreign origin query")
                .is_empty(),
            "opaque context must never cross a provider-instance boundary"
        );
        let opaque_marker = "opaque-reasoning";
        let public_rows: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM messages WHERE payload LIKE ? OR search_text LIKE ?",
        )
        .bind(format!("%{opaque_marker}%"))
        .bind(format!("%{opaque_marker}%"))
        .fetch_one(reopened.pool())
        .await
        .expect("check public transcript projections");
        assert_eq!(
            public_rows, 0,
            "opaque bytes leaked to public transcript/FTS input"
        );
