...
- やること: §5.1 疑似コードの忠実実装。Length 一括失敗(#19)+**同一 run 内連続2回で3回目へ進まない暴走ガード**、sequential 実行固定、steering ポーリング位置(#18)、one-at-a-time(#23)、run 失敗時の合成エラーで正常形クローズ(#22)。Length中のvalid ToolCallは公開assistantへ残し、private `Skip`で`not_started/length_guard`監査行+error ToolResult対だけを同一transactionへ保存する(承認/executor/公開ToolExecution lifecycleなし)。Session は**制御プレーンと run ワーカーを分離した actor**(RunCore move、二重 select 必須 — 単純 await 禁止、§5.2)。RunCore由来のprivate metadata-bound outputをstrict EventBatchへ変換し、durable commitと返却seqの確定後だけGatewayへ渡す。リトライ実施(T7 のポリシー+`MessageEnd(error)→RetryScheduled→次attempt` のイベント列、error assistant は `append_to_l0=false`)。control 直列化境界と **Abort cutoff**(§5.2: seq 順終端、後続 seq を先に ACK しない)、steer group の分類・snapshot・一括注入(bounded window、T12 の sizer)、run owner 移譲(付録C 行11)。T8の計測口へcommand受信時刻を接続し、`command受信→request_sent`/`request_sent→first Text|Thinking delta`を分離、stdio表示と内部overhead p95判定を完成する。承認は本物を作らず **fixture 専用 Pending action driver** で `ApprovalRequested+approval_log.pending`/解決/復旧 transaction だけ検証(§13 M2)。ツール実行開始は `ToolExecutionStart+prepared→running` の transaction を通す(§10.2)
- 受け入れ: 実Store/Sessionでcommit-before-deliveryと返却seq、batch rollback時にpublic frameゼロ、MessageEnd完全一致、通常tool lifecycle対、first-Lengthのnot_started+result対、retry error除外+RetryScheduled、provider_context非空のfail-closedを固定する。remote executor adapterはartifact pathだけRPC limitをcontinuation予約込みでpre-capし、page/artifact EOFとexact byte cursor detailsを区別し、production-valid fakeが要求RPC limitを超えないことを固定する。stdio E2E(tests/loop_e2e.rs: モックプロバイダで user_message→イベント列アサート、異常系込み正常形クローズ)。M2 ゲート1・4・5・6c の該当部分(steer 前の soft 分岐は T16 と分担可)。実プロバイダなしで全緑
