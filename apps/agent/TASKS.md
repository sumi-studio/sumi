...
- 受け入れ: M1 ゲート1(strict/preview 分離、reasoning 分離、usage、z.ai 固有 finish_reason 込み)を cargo test で。OpenCode Zen Goはcurl raw captureをsanitization前SHA-256とmetadata付きで固定し、`[DONE]`後cost trailerも保持する。Moonshot/Z.ai/Umans directはadapter互換性の任意な開発証拠であり、Cloud releaseのlive gateには使わない。T25へ引き継ぐrelease-blocking live証拠はResponses経路だけとする。adapter正規化だけのp95 smokeを置くが、command受信からrequest送出までを含む内部オーバーヘッド p95<30ms の正式判定は T15。通常channel飽和中でもabort 1s以内、partial+usage保持、terminal後fuse(M1-3)
...
- やること: Responses encrypted reasoning/ordered compact outputとAnthropic signature/redacted thinking/compaction blockを、暗号化provider_contextへMessageEndと同一transactionで保存し、再起動後に同じprovider instance/protocol/model/fingerprintへ順序不変で再送する。異なるtrust domainへの送信はfail-closed。Chat Completions / Responses / Anthropicの3 protocolは公式またはprovenance付きfixtureで契約を固定する。Cloud releaseの実ライブ経路はResponsesだけとし、Codex Responses bridgeを通るtool 1往復、kill/restartを挟む2ターン、encrypted reasoning再送、native compactionを同じrelease profileで実証する
- 受け入れ: M1Pのdurable round-trip・tool-use継続・cache usage・native compaction全ゲート。Responses live経路でkill/restartを挟んだ2ターン目が成功し、公開transcript/FTS/exportにopaque byteが出ない。`SUMI_LIVE_TEST=1`のT25 release dispatcherはResponsesだけを選び、必要なResponses credential・bridge設定の不足や空値はlive gate失敗にする。credential不要の通常テストと3 protocolのfixture/contractテストはlive実行と分離する。Moonshot/Z.ai/Umans/OpenCodeのlive captureは任意な開発証拠であり、このResponses release証拠の代替にも追加必須条件にもならない
...
1. **provider release (T25)**: M1P の Responses/Anthropic fixture・contractゲートと、Responses-only live releaseゲートを完了(T9/T10 + T17 後の durable round-trip 結合)
