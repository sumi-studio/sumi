...
- 2026-07-28 identity/lifecycle cutover: 完了済みtaskの履歴・完了状態は維持するが、実装・fixtureに残るlegacy `agent_id`／`conversation_id`はglobal UUIDv7 `PersonalityAgentId`へのpre-launch置換対象であり、aliasや互換contractではない。以下の受け入れはcutover後の正典を記述する
...
- 読む: §10 冒頭(鍵階層・AAD 契約・versioned envelope)、§10.1 のうち `personality_agent_scope` / `data_keys` / `messages` / `agent_events` / `inbound_commands` / `tool_executions` / `approval_log`、§13 M2 の「M2で data_keys を含める理由」
...
- やること: global lowercase-hyphenated UUIDv7 `PersonalityAgentId`のtyped validatorと`personality_agent_scope`、KeyProvider trait(Cloud=KMS、ローカルは環境変数テスト鍵。§10「鍵の供給」)、agent-owned用途別/retention-unit別data keyのAEAD wrap/unwrap(`data_keys`表がwrapの正典、crypto-erase=`state='destroyed'`+wrapped_key破棄)、暗号文のversioned envelope形式`version(1B)||nonce(24B)||ct+tag`(XChaCha20-Poly1305、OsRng)を実装する。tenant KEKは置換可能なouter agent-key wrapでありowner identityではない。**AADは行の実位置から再構成**し、owner成分をcanonical `PersonalityAgentId`、残りをtable/行id/key_ref/purpose/schema version/必要なimmutable event-time provenanceとする。mutableなtenant／Workspace／org membershipをagent-private AADへ焼き込まない。provider contextのmessage/itemまたはcoverage-window単位、artifact class/handle単位のkey-refと独立crypto-erasureをowner統合で一つへ潰さない。Redactor基盤は原文暗号化正本+redacted projection+`redaction_version`を**同時生成**するversioned純関数とする(全secret pattern網羅はM3、ここでは基盤+固定fixture)
- 受け入れ: M2ゲート9(non-v7/noncanonical ID、legacy field、未知purpose、空retention unit、不正state fixtureを全拒否)、行スワップ(2行間のciphertext/key_ref入替)で復号が必ず拒否されるfixture、outer agent-key wrapの交換でagent-private row ciphertextが変わらないfixture、M3ゲート9の`inbound_commands`/`tool_executions` CHECK fixtureの先行分
...
- やること: global UUIDv7 `PersonalityAgentId`を宛先/ownerに持つEnvelopeとCommandEnvelope(durable eventはseq必須/delta・Errorはseq禁止のif/then)、AgentEvent全variant、CommandAck(rejectedのみreject_reason)、OutboundFrame、`attachments: maxItems 0`。**user message_id はcanonical `PersonalityAgentId`をnamespaceとするUUIDv5導出をcontractsに正典として明記**。legacy `agent_id`／`conversation_id` fieldや独立conversation scopeは生成しない。内部 `ProviderEvent` へ variant を足しても明示変換を更新しない限り wire に出ない構造(契約ファースト、§11.2 末尾)
- 受け入れ: canonical UUIDv7だけを通しlegacy/noncanonical IDを拒否するRust round-trip fixture(non-empty attachments拒否含む)。**この時点で wire 形を凍結し、api/web 担当に contracts を渡す**(Go/TS 型生成と3言語 round-trip CI は api 側と合同)
...
- やること: provider-visible schemaをADR 0013の必須`route + requested_authority + authority_binding_ref + input` envelopeにし、strict検証済み`ToolCall`へimmutable route・requested authority・opaque binding refを固定する。NormalはAgentOwn+null、ElevatedはAgentOwn+nullまたはHumanAccountOneShot+non-nullだけを許し、provider replayでも同じenvelopeを決定論的に再構築する。policyがagentに別のElevated proposalを要求できるかも先に決定し、どちらでも既存Normal callの途中変換・replayやDeny/BlockからHuman promptへのfallbackは実装しない。`CanonicalAction`はruntime内部正本でwire/DBへraw保存しない。Normal policyはexplicit `Allow | Deny | Unmatched`だけを返す。shell複合commandをsegment分解し、どれかexplicit DenyならDeny、全segment explicit AllowならAllow、それ以外(heredoc・動的eval・解析不能を含む)はUnmatched。managed hard deny/sandbox/app commit時認可は別の非override境界にする。`SecretAwareActionProjector`はsecretをkind+keyed digestへ置換し、判定材料不足を`InsufficientEvidence`にする。standing policyはcurrent-call approvalと別contractであり、旧`ApproveAlways`/`NeedsApproval`をNormal latticeへ残さない
- 受け入れ: envelopeの欠落/未知field/不正なroute-authority-binding組/途中変更をfail-closed、NormalのAllow/Deny/Unmatchedをshell fixtureで固定、explicit Denyでreviewer/Human prompt 0件、raw action/secretがwire・DB・reviewerへ出ないこと。binding issuer / registryが無いprovider snapshotでは`human_account_one_shot`をadvertiseせず、非advertise値やunknown/stale/revoked/mismatched refは`AuthorityBindingUnavailable`のdurable `RejectedToolCall`+synthetic `is_error` resultで閉じ、approval / execution row、reviewer、Human promptを0件にする。policy missing/staleの挙動とstanding ruleのscope/precedence/expiryは未決を解消してからfixture化する
...
- やること: global canonical UUIDv7 `PersonalityAgentId`をdeployment/private-state owner keyにし、T13Bの中立共有型を使ってpersistent monotonic allocatorを唯一の発行元として`ProcessGeneration` leaseをruntime起動前に払い出し、runtime/executor/artifact brokerへ同じ値を配布する。allocator/current-generation fence/Ready registryはadministrative contextを含めず`PersonalityAgentId`単位で一つだけ持つ。T26は型を再定義せず、durable allocator/issuanceとproduction lease acquisitionを所有する。domainは0を含む`0..=i64::MAX`とし、increment前に最大値を検査して`i64::MAX`後はwrap/reuseせず新bootstrapをfail-closedに拒否する。`PersonalityAgentId`ごとのdedicated VM内でruntime/executor/brokerを別UID・別mount・別network namespaceで起動し、executor/broker RPC専用の`RpcBootNonce`を`ProcessGeneration`と対で払い出す。T24 Gateway credential、T17 Store scope/`GenerationRecoveryFence`、Session、executor/broker RPCへ同じ`ProcessGeneration`を束縛し、値の欠落・不一致を開始前に拒否する。T17のtyped `HydratedRunState`/recovery intents/hydration receipt、T21の`ThreeLayerMemory`/`ContextAssembler`、T23の`ApprovalBroker`、production `ToolRegistry`、provider、Gateway、executor境界を明示的なbootstrap moduleで組み立てる、唯一のproduction `RunCore` composition boundaryとする。current generationのhydration stateを`NotReady`で作成し、構成完了後にだけT17のstable `HydrationReceiptIdentity`へ束縛したimmutableな`Ready { generation, hydration_receipt_identity }`をlatchする。generation rolloverでは旧Readyを新generationの公開前または同じatomic state transitionでinvalidateし、新generationを必ずNotReadyから始める。旧generationのlate Readyは拒否する。T24がhello後に観測したcurrent Readyに限りbounded holdしたcommandをSessionへ公開してからproduction `main`をM0 admission echo shellから差し替える。ready前はcommand ACK/provider/executor 0件とし、hold上限超過はfail-closedにする。identityをenv/defaultで補わず、空history/provider context、no-tool、newly-provisioned-onlyへ縮退しない。executor/brokerは`network_mode=none`、runtimeはLLM通信を維持する。artifactの0600/0700、canonical-ID path validation、fd-relative no-followとper-agent private namespaceをproduction構成で保証する。current verticalは一つのauthorization contextでよく、membership/transfer/shared-Workspace実装を追加しない。旧世代の物理kill/reap、resource quota、descendant cleanup、crash recoveryはT27に残す
- 復旧順序: clean existing agentでT17が返すphysical recovery intentsが空なら、T26のlease/`GenerationRecoveryFence`だけでcompositionを続行できる。非空なら`HydrationReady`を発行せずT27 integrationまでfail-closedにし、T27の`PhysicalRecoveryReceipt`をT17へ再注入して、T17 application ledger・影響suffix・terminalの同一transactionが完了したreceiptを得てから再開する。同一receipt ID+digest+lease+canonical exact intent setの再注入はledgerの完全一致時だけalready-appliedへ収束し、競合・stale・異なるdigestのID再利用は拒否する。これによりT26のclean bootstrap acceptanceはT27へ循環依存しない
- 受け入れ: 各componentから禁止path/socket/networkが読めないこと、bash子がbrokerへ到達できないこと、file toolは同一executor UIDで継続できることを自動テスト。同じWorkspaceかつ同じadministrative contextの二agent fixtureでVM、DB、private workspace、artifact volume、IPC、credential、failure domainが分離されることを固定する。認証済み既存agentをproduction `main`で起動し、T17復元済みanchor/context、完成したThreeLayerMemory/ApprovalBroker/ToolRegistry、同一`ProcessGeneration`と対になる`RpcBootNonce`を使ってGateway commandからprovider/toolまで到達する。allocatorのgeneration 0と`i64::MAX`の起動・永続化、`i64::MAX`後の拒否、全componentへの配布と各境界のmismatchを固定する。T26所有fixtureとしてcurrent generationのNotReady初期化、stable hydration receipt identityへ束縛した一度だけのReady latch、generation rollover時の旧Ready invalidation、新generationのNotReady開始、stale旧generation Ready拒否を単一の決定論的state-machine fixtureで固定する。T24のready-before/hello-before fixturesとT28のready-after-reconnect fixtureは複製しない。ready前のACK/provider/executorは0件で、identity/context/fence/generation/nonceの不整合時はSession/provider/executor開始前にfail-closedとなる。M0 echo、M2注入mock、newly-provisioned agentだけの成功は代替しない
...
- 変更対象: store migration/lifecycle worker、API life-log export/agent-death deletion、artifact tombstone/GC、KMS KeyProvider、runbook/backup fixture
- やること: `PersonalityAgentId`へ束縛したcanonical life-log/artifact/private-workspace export、supervisor-owned agent death、provider context/memory projectionの独立retention/crypto-erase、artifact class/handle単位のtool-output bounded GC、検索監査を実装する。provider-context message/item・coverage-window keyとartifact key-refの粒度は維持し、owner/AADだけをglobal `PersonalityAgentId`へ統合する。active L0/provider inputから参照されるattachmentは必要な間pinする。agent deathは外部`deletion_tombstones`の`requested → fenced → live_purged → backup_expired` state machineで一つのcurrent generationをfenceし、agent DB/canonical life log/全key/private workspace/artifact volume/VM credential/backupを破棄する。同じidentityへ新sessionやseqを発行せず、後継は新しい`PersonalityAgentId`としてprovisionする。backup復元時もtombstoneを先に再適用する。環境変数テスト鍵からKMSへ移行し、tenant KEKを置換可能なouter agent-key wrapとしてhistory-preserving rewrap/rotationとrevocationを扱う。選択的忘却・redaction・法的retentionは対象class、authority、typed tombstone/provenance、auditを持つ別semanticsなしに実行せず、通常のreset成功へ落とさない
- 受け入れ: lifecycle各段階kill/restart、逆行/飛越し拒否、backup再適用、history-preserving KMS rewrap/rotationと失効後拒否。同じWorkspaceかつ同じadministrative contextの二agent isolation fixtureで、physical recovery適用済み対象agentのDB/ledger/event/execution/private workspace/artifact subtree/keyだけが消え、第二agentのprivate stateは不変であることを固定する。tool-output payload GC、provider-context item/window crypto-erasure、agent deathが互いのkey-refを巻き込まないこと、pinned input attachmentがtool-output GCで消えないことを固定する。delta分割secretとCompact結果を含むredaction fixtureでredaction-only接続・FTS・exportから復元不能とする。typed/audited contractのない選択的erasure要求はfail-closedに拒否する
...
2. **production runtime bootstrap + per-agent executor/artifact deployment (T26)**: global `PersonalityAgentId`をowner keyにし、T13Bの中立共有型を使い、T26がpersistent monotonic allocator/issuanceとproduction lease acquisitionから発行した一つのcurrent `ProcessGeneration`をT24 Gateway credential、T17 Store hydration、Session、executor/brokerへ束縛する。T17のtyped `HydratedRunState`/recovery intents/stable identity付きhydration receipt、T21のThreeLayerMemory、T23のApprovalBroker、production ToolRegistryから唯一のRunCoreをproduction `main`で構成する。T26単独acceptanceはphysical intentsが空のclean existing agentを使い、非空ならT27の`PhysicalRecoveryReceipt`再注入までfail-closedにする。`RpcBootNonce`はexecutor/broker RPC専用として同generationと対にする。hydration stateはgenerationごとのNotReady→immutable Ready latchで、rollover前/同時に旧Readyをinvalidateし、各helloはcurrent stateを観測する。NotReady中はbounded hold/backpressureまたはfail-closedで、ready前のACK/provider/executorは禁止する。same-Workspace/same-context二agentのVM/private-state isolation、sidecarの別UID・`network_mode=none`・volume mount分離を満たし、M0 echo、空context/no-tool、newly-provisioned-only経路は代替不可
...
6. **data lifecycle (T29)**: `PersonalityAgentId`へ束縛したlife-log export、agent death、派生memory/provider-context/artifact payloadの独立retention/crypto-erase、tombstone再適用、typed/audited contractなしの選択的erasure拒否、redaction fixture(delta分割secret含む)、replaceable tenant-KEK outer wrapを持つKMS移行、運用runbook
