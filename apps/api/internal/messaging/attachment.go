...
// AttachmentPolicy is the operator-owned attachment configuration. A zero
// WorkspaceQuotaBytes fails closed: attachments stay disabled rather than
// running unlimited.
...
// resolves the per-file nonce receipt, and reserves the declared bytes against
// the Workspace ledger and the uploader's per-place draft budget. No body byte
// may be read before this returns a reservation.
func (s *ScopedStore) ReserveAttachmentUpload(ctx context.Context, placeID, clientNonce string, declaredBytes int64) (AttachmentUploadReceipt, error) {
	...
	// The Workspace ledger row serializes reservations per Workspace so the cap
	// and the per-uploader draft budget are decided against a locked total.
	used, err := lockWorkspaceQuota(ctx, tx, s.Scope.WorkspaceID)
	if err != nil {
		return AttachmentUploadReceipt{}, err
	}
	now := time.Now()
	...
	).Scan(&uploadID, &state, &declared, &expiresAt)
	switch {
	...
	if uploadID != "" {
		...
		case "finalized":
			...
			if !found {
				return AttachmentUploadReceipt{}, errors.New("finalized attachment reservation has no attachment row")
			}
			if existing.SizeBytes != declaredBytes {
				return AttachmentUploadReceipt{}, ErrAttachmentUploadConflict
			}
		...
		case "reserved":
			...
			// the ledger already holds its bytes.
			...
			if _, err := tx.Exec(ctx, `
				...
				WHERE upload_id = $1`,
				uploadID, s.Scope.InstallationID, s.Scope.AuthorityEpoch, expiresAt); err != nil {
			...
			return AttachmentUploadReceipt{Reservation: &AttachmentUploadReservation{
				...
				DeclaredBytes: declaredBytes, ExpiresAt: expiresAt,
			}}, nil
		case "released":
			if declared != declaredBytes {
				return AttachmentUploadReceipt{}, ErrAttachmentUploadConflict
	...
	if used+declaredBytes > policy.WorkspaceQuotaBytes {
	...
	if uploadID == "" {
		...
		if _, err := tx.Exec(ctx, `
			INSERT INTO message_attachment_uploads
				(upload_id, workspace_id, place_id, uploader_kind, uploader_id, client_nonce,
				 installation_id, authority_epoch, declared_bytes, state, expires_at)
		...
		SET state = 'reserved', settled_at = NULL, installation_id = $2,
		    authority_epoch = $3, expires_at = $4
		WHERE upload_id = $1`,
		uploadID, s.Scope.InstallationID, s.Scope.AuthorityEpoch, expiresAt); err != nil {
		return AttachmentUploadReceipt{}, fmt.Errorf("re-reserve attachment upload: %w", err)
	}
	if err := adjustWorkspaceQuota(ctx, tx, s.Scope.WorkspaceID, declaredBytes); err != nil {
		return AttachmentUploadReceipt{}, err
	}
	if err := tx.Commit(ctx); err != nil {
...
// lockWorkspaceQuota creates the ledger row on first use and returns the
// locked used_bytes total.
func lockWorkspaceQuota(ctx context.Context, tx pgx.Tx, workspaceID string) (int64, error) {
	if _, err := tx.Exec(ctx, `
		INSERT INTO message_attachment_quotas (workspace_id) VALUES ($1)
		ON CONFLICT (workspace_id) DO NOTHING`, workspaceID); err != nil {
		return 0, fmt.Errorf("ensure attachment quota row: %w", err)
	}
	var used int64
	if err := tx.QueryRow(ctx, `
		SELECT used_bytes FROM message_attachment_quotas WHERE workspace_id = $1 FOR UPDATE`,
		workspaceID).Scan(&used); err != nil {
		return 0, fmt.Errorf("lock attachment quota: %w", err)
	}
	return used, nil
}

func adjustWorkspaceQuota(ctx context.Context, tx pgx.Tx, workspaceID string, delta int64) error {
	tag, err := tx.Exec(ctx, `
		UPDATE message_attachment_quotas
		SET used_bytes = used_bytes + $2, updated_at = now()
		WHERE workspace_id = $1`, workspaceID, delta)
	if err != nil {
		return fmt.Errorf("adjust attachment quota by %d: %w", delta, err)
	}
...
func (s *ScopedStore) FinalizeAttachmentUpload(ctx context.Context, placeID string, staged StagedAttachment) (Attachment, bool, error) {
	if err := s.requireAttachments(); err != nil {
		return Attachment{}, false, err
	}
	blobs := s.Store.blobs
	...
	).Scan(&uploadID, &state, &declared, &installation, &epoch, &expiresAt, &clientNonce)
	if errors.Is(err, pgx.ErrNoRows) {
	...
	case "finalized":
		...
		if !found {
			return discard(finalizeNotCommitted(errors.New("finalized attachment reservation has no attachment row")))
		}
		if err := tx.Commit(ctx); err != nil {
