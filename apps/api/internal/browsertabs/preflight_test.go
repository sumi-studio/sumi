...
func TestBrowserInvalidElevatedCallDoesNotAskForApproval(t *testing.T) {
	...
	cases := []struct {
		...
		{"missing attachment", "browser.observe", map[string]any{}, false},
		{"invalid action", "browser.act", map[string]any{"attachment_id": id}, false},
		{"oversize action", "browser.act", map[string]any{"attachment_id": id, "binding": map[string]any{"url": strings.Repeat("x", 33<<10)}}, false},
		{"valid explicit elevated", "browser.observe", map[string]any{"attachment_id": id}, true},
	}
