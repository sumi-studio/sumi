...
const APIProtocols = new Set(["open_ai_chat_completions", "open_ai_responses", "anthropic_messages"]);
const ToolArgumentErrors = new Set(["invalid_json", "non_object", "schema_violation", "incomplete_response", "too_large"]);
...
function isSafeAnyJSON(value: unknown): boolean {
  ...
  while (pending.length > 0) {
    ...
    if (typeof current === "number") {
      if (!Number.isFinite(current) || Math.abs(current) > Number.MAX_SAFE_INTEGER) return false;
      continue;
    }
...
function isUserContent(value: unknown): boolean {
  ...
  if (value.type === "text") {
    return hasRequiredAndOnlyKeys(value, ["type", "text"]) && typeof value.text === "string";
  }
  if (value.type === "image") {
    return hasRequiredAndOnlyKeys(value, ["type", "data", "mime_type"]) &&
      typeof value.data === "string" && typeof value.mime_type === "string";
  }
  return false;
}
...
function isToolCall(value: unknown): boolean {
  return isRecord(value) &&
    ...
    isRecord(value.arguments) && isSafeAnyJSON(value.arguments);
}
...
function isRejectedToolCall(value: unknown): boolean {
  return isRecord(value) &&
    ...
    typeof value.id === "string" && typeof value.name === "string" &&
    typeof value.error === "string" && ToolArgumentErrors.has(value.error);
}

function isAssistantContent(value: unknown): boolean {
  if (!isRecord(value) || typeof value.type !== "string") return false;
  if (value.type === "text") {
    return hasRequiredAndOnlyKeys(value, ["type", "text", "wire_item_index"]) &&
      typeof value.text === "string" && isSafeSequence(value.wire_item_index);
  }
  if (value.type === "thinking") {
    return hasRequiredAndOnlyKeys(value, ["type", "thinking", "signature_field", "wire_item_index"]) &&
      typeof value.thinking === "string" && typeof value.signature_field === "string" &&
      isSafeSequence(value.wire_item_index);
  }
  if (value.type === "tool_call") {
    return hasRequiredAndOnlyKeys(value, ["type", "tool_call", "wire_item_index"]) &&
      isToolCall(value.tool_call) && isSafeSequence(value.wire_item_index);
  }
  if (value.type === "rejected_tool_call") {
    return hasRequiredAndOnlyKeys(value, ["type", "rejected", "wire_item_index"]) &&
      isRejectedToolCall(value.rejected) && isSafeSequence(value.wire_item_index);
  }
  return false;
}
...
function isProviderOrigin(value: unknown): boolean {
  return isRecord(value) &&
    hasRequiredAndOnlyKeys(value, ["provider_instance_id", "protocol", "model"]) &&
    typeof value.provider_instance_id === "string" &&
    typeof value.protocol === "string" && APIProtocols.has(value.protocol) &&
    typeof value.model === "string";
}

function isUsage(value: unknown): boolean {
  const keys = ["input", "output", "cache_read", "cache_write", "reasoning", "total_tokens"];
  return isRecord(value) && hasRequiredAndOnlyKeys(value, keys) &&
...
function isPublicMessage(value: unknown): boolean {
  ...
  if (value.role === "user") {
    return hasRequiredAndOnlyKeys(value, ["role", "content", "timestamp"]) &&
  ...
  if (value.role === "assistant") {
    return hasRequiredAndOnlyKeys(value, [
      "role", "content", "model", "provider", "origin", "usage", "stop_reason",
      "error_message", "provider_code", "interrupted", "timestamp",
    ]) &&
      Array.isArray(value.content) && value.content.every(isAssistantContent) &&
      typeof value.model === "string" && typeof value.provider === "string" &&
      isProviderOrigin(value.origin) && isUsage(value.usage) &&
      typeof value.stop_reason === "string" && StopReasons.has(value.stop_reason) &&
      isStringOrNull(value.error_message) && isStringOrNull(value.provider_code) &&
      typeof value.interrupted === "boolean" && isDateTime(value.timestamp);
  }
  if (value.role === "tool_result") {
    return hasRequiredAndOnlyKeys(value, [
      "role", "tool_call_id", "tool_name", "content", "details", "is_error", "timestamp",
    ]) &&
      typeof value.tool_call_id === "string" && typeof value.tool_name === "string" &&
      Array.isArray(value.content) && value.content.every(isUserContent) &&
      isSafeAnyJSON(value.details) && typeof value.is_error === "boolean" &&
      isDateTime(value.timestamp);
  }
  return false;
}
...
function isPublicStreamEvent(value: unknown): boolean {
  ...
  if (StreamStartTypes.has(value.type)) {
    return hasRequiredAndOnlyKeys(value, ["type", "content_index"]) &&
      isSafeSequence(value.content_index);
  }
  if (StreamDeltaTypes.has(value.type)) {
    return hasRequiredAndOnlyKeys(value, ["type", "content_index", "delta"]) &&
      isSafeSequence(value.content_index) && typeof value.delta === "string";
  }
  if (StreamEndTypes.has(value.type)) {
    return hasRequiredAndOnlyKeys(value, ["type", "content_index", "content"]) &&
      isSafeSequence(value.content_index) && typeof value.content === "string";
  }
  if (value.type === "tool_call_preview") {
    return hasRequiredAndOnlyKeys(value, ["type", "content_index", "preview"]) &&
      isSafeSequence(value.content_index) && isSafeAnyJSON(value.preview);
  }
  if (value.type === "tool_call_end") {
    return hasRequiredAndOnlyKeys(value, ["type", "content_index", "tool_call"]) &&
      isSafeSequence(value.content_index) && isToolCall(value.tool_call);
  }
  if (value.type === "tool_call_rejected") {
    return hasRequiredAndOnlyKeys(value, ["type", "content_index", "rejected"]) &&
      isSafeSequence(value.content_index) && isRejectedToolCall(value.rejected);
  }
  return false;
}

function isReviewProjection(value: unknown): boolean {
  if (!isRecord(value) || !hasOnlyKeys(value, ["reviewable", "insufficient_evidence"])) return false;
  ...
  return isRecord(value.insufficient_evidence) &&
...
function isAuditDecision(value: unknown): boolean {
  return isRecord(value) &&
    hasRequiredAndOnlyKeys(value, ["outcome", "risk", "authorization", "rationale"]) &&
    typeof value.outcome === "string" && AuditOutcomes.has(value.outcome) &&
    typeof value.risk === "string" && RiskLevels.has(value.risk) &&
    typeof value.authorization === "string" && UserAuthorizations.has(value.authorization) &&
...
function isApprovalRequest(value: unknown): boolean {
  return isRecord(value) &&
    hasRequiredAndOnlyKeys(
      ...
      ["id", "tool_call_id", "tool_name", "action", "args_summary", "reason", "audit"],
    ) &&
    typeof value.id === "string" && typeof value.tool_call_id === "string" &&
    typeof value.tool_name === "string" && isReviewProjection(value.action) &&
    isSafeAnyJSON(value.args_summary) &&
    (!("reason" in value) || isStringOrNull(value.reason)) &&
    (!("audit" in value) || value.audit === null || isAuditDecision(value.audit));
}
...
function isApprovalDecision(value: unknown): boolean {
  ...
  return value.type === "approve_always" &&
    hasRequiredAndOnlyKeys(value, ["type", "rule"]) &&
    isRecord(value.rule) && isSafeAnyJSON(value.rule);
}

function isApprovalResolution(value: unknown): boolean {
  return value === "cancelled" ||
    (isRecord(value) && hasRequiredAndOnlyKeys(value, ["decision"]) &&
...
function isSafeEventForUI(
  ...
  if (value.type === "agent_start" || value.type === "agent_end" || value.type === "turn_start") {
    return hasRequiredAndOnlyKeys(value, ["type"]);
  }
  if (value.type === "turn_end") {
    return hasRequiredAndOnlyKeys(value, ["type", "message", "tool_results"]) &&
      (value.message === null || isPublicMessage(value.message)) &&
      Array.isArray(value.tool_results) && value.tool_results.every(isToolResultPayload);
  }
  ...
  if (value.type === "tool_execution_start") {
    return hasRequiredAndOnlyKeys(value, ["type", "tool_call_id", "tool_name", "args"]) &&
      typeof value.tool_call_id === "string" && typeof value.tool_name === "string" &&
      isRecord(value.args) && isSafeAnyJSON(value.args);
  }
  if (value.type === "tool_execution_update") {
    return hasRequiredAndOnlyKeys(value, ["type", "tool_call_id", "partial"]) &&
      typeof value.tool_call_id === "string" && isSafeAnyJSON(value.partial);
  }
  if (value.type === "tool_execution_end") {
    return hasRequiredAndOnlyKeys(value, ["type", "tool_call_id", "result", "is_error"]) &&
      typeof value.tool_call_id === "string" && isSafeAnyJSON(value.result) &&
      typeof value.is_error === "boolean";
  }
  if (value.type === "steered") {
    return hasRequiredAndOnlyKeys(value, ["type", "mode"]) &&
      (value.mode === "hard" || value.mode === "soft");
  }
  if (value.type === "approval_requested") {
    return hasRequiredAndOnlyKeys(value, ["type", "request"]) &&
      isApprovalRequest(value.request);
  }
  if (value.type === "approval_resolved") {
    return hasRequiredAndOnlyKeys(value, ["type", "request_id", "resolution"]) &&
      typeof value.request_id === "string" && isApprovalResolution(value.resolution);
  }
  if (value.type === "memory_maintenance") {
    return hasRequiredAndOnlyKeys(value, ["type", "kind"]) && typeof value.kind === "string";
  }
  if (value.type === "retry_scheduled") {
    return hasRequiredAndOnlyKeys(value, ["type", "attempt", "delay_ms", "retry_at", "error_message"]) &&
      isSafeSequence(value.attempt) && isSafeSequence(value.delay_ms) &&
      isDateTime(value.retry_at) && typeof value.error_message === "string";
  }
  if (value.type === "error") {
    return hasRequiredAndOnlyKeys(value, ["type", "message"]) &&
