...
export class DirectChatTimeline {
  ...
  apply(frame: DirectChatServerFrame): FeedItem[] {
    ...
    if (type === "message_start" || type === "message_end") {
      ...
      if (role && text)
        this.replace({ id: `message-${messageID}`, kind: role, text });
      if (type === "message_end") {
      ...
      this.replace({
        ...
        text:
          type === "tool_execution_start"
            ...
            : `Tool finished: ${callID}`,
      });
    } else if (type === "steered") {
      const seq = "seq" in frame.envelope ? frame.envelope.seq : "volatile";
      this.appendOnce(`event-${seq}`, {
        id: `event-${seq}`,
        kind: "steer",
  ...
  private appendOnce(id: string, item: FeedItem) {
    if (!this.feed.some((entry) => entry.id === id))
      this.feed = [...this.feed, item];
  }
  private replace(item: FeedItem) {
    this.feed = [...this.feed.filter((entry) => entry.id !== item.id), item];
  }
