/**
 * 通話のREST境界。MessagingBackendとは別に持つ——通話はメディアの層であって、
 * メッセージのdurable truthを運ぶbackendではない（ADR 0012）。
 * SFUが未設定のdeploymentではcalls_unavailableが返り、通話ボタンだけが
 * 失敗してテキストはそのまま動く。
 */
...
export class CallAPIError extends Error {
  ...
  /** SFUが立っていないだけなのか、権限で断られたのかをUIが言い分けるため。 */
...
/** placeの部屋へ入る切符を貰う。membershipはサーバーが判定する。 */
...
/**
 * 今開いている通話の一覧。call_stateイベントはreplayされないvolatileなので、
 * 接続直後の現在値はここでしか手に入らない。
 */
...
function asString(value: unknown): string {
  if (typeof value !== "string") throw new Error("invalid call response");
  return value;
}
