...
/**
 * メッセージングのオーバーレイ（ポップオーバー/メニュー/浮かせたカード）が
 * 共通で守る規律をここに閉じ込める。
 *
 * - パネルの外をクリック、または Escape で閉じる
 * - 同時に開くのは 1 つだけ（後から開いたものが前のものを閉じる）
 * - 閉じたらトリガーへフォーカスを戻す
 * - トリガー上の pointerdown では閉じない（閉じると直後の click で開き直る）
 * - パネルの上でのホイールは、パネル自身に独立したスクロール領域が
 *   ない限り下のメッセージ一覧へ渡す
 *
 * ポップオーバーの上に不可視の膜（backdrop）を敷かないのも規律のうち。
 * 膜はホイールもポインタも奪い、下の一覧が動かなくなる。
 */

/** 開いているオーバーレイの「閉じる」手続き。排他のために 1 か所へ集める。 */
const openOverlays = new Set<() => void>();

/** メッセージ一覧のスクロール領域。オーバーレイ上のホイールを渡す既定の相手。 */
...
/**
 * 下のスクロール領域へホイールを透過させる ref。
 * portal 越しに後から現れる要素にも効くよう ref callback で受け渡す。
 */
...
export function useOverlayPanel<T extends HTMLElement = HTMLButtonElement>({
  ...
  // 閉じたときにフォーカスをトリガーへ戻すか。外側クリックで閉じた場合は
  // 押した先へフォーカスが移るので奪い返さない。
  ...
  // フォーカスがパネル/トリガーの内側にあるかを追う（復帰の判断に使う）。
  useEffect(() => {
    if (!open) return;
